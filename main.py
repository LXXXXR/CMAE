...
def get_args():
    ...
    parser.add_argument(
        "--exp_mode", type=str, default="active_cen", help="active_cen|epsilon|bonus"
    )
    parser.add_argument(
        "--env",
        type=str,
        default="room",
        help="room|room30|secret_room|push_box|island",
    )
    ...
    parser.add_argument(
        "--stochastic_select_subspace", default=False, action="store_true"
    )
    ...
    parser.add_argument(
        "--alpha1", type=float, default=0.1, help="learning rate for exploration policy"
    )
    parser.add_argument(
        "--alpha2", type=float, default=0.05, help="learning rate for active policy"
    )
    parser.add_argument(
        "--recip_t", type=float, default=50, help="reciprocal temperature"
    )
    parser.add_argument(
        "--subspace_q_size", type=int, default=10, help="subspace_q_size"
    )
...
def main():
    # params
    args = get_args()
    log_path = "log/{}/{}/{}_{}".format(
        args.log_folder, args.exp_batch_name, args.exp_name, args.seed
    )
    ...
    wandb.init(project=args.env, entity="lxxxxr", config=kargs)
    ...
    s_raw = env.reset()
    # convert multi-dim obs to a single huge int
    ...
    while True:
        ...
        if (
            "active" in args.exp_mode
            and episode_count >= 1
            and (
                s == goal_s
                or s == s_to_sp(goal_s, base=env.grid_size, raw_dim=raw_obs_dim)
            )
        ):
            steps_to_goal = episode_step
        s_next_raw, r, done = env.step([a1, a2])
        s_next, s_next_p = obs_to_int_pi(
            s_next_raw, base=env.grid_size, raw_dim=raw_obs_dim
        )
        if "active" in args.exp_mode:
        ...
        if done:
            ...
            if episode_count % 20 == 0:
                print(
                    "episode {}, rew {}, avg. rew {}, n_new_state_action avg {:.4f}, step to goal {}, eps {}".format(
                        episode_count,
                        episode_rew,
                        np.mean(episode_rews),
            ...
            if "active" in args.exp_mode:
                goal_s, goal_a = meta_q.get_goal(
                    multilevel=args.multilevel, all_subspace=args.all_subspace
                )
                goal_a1, goal_a2 = divmod(goal_a, n_actions_per_agent)
                if episode_count % 20 == 0:
                    print(
                        "Restricted space level {},  id {}. Goal state  {}, Goal action {}".format(
                            meta_q.level,
                            meta_q.count_id,
                            convert_to_base(goal_s, env.grid_size),
                            goal_a,
                        )
                    )
            ...
            if episode_count % args.eval_every_episode == 1:
                ...
                with open(os.path.join(log_path, "data.csv"), "w") as f:
                    ...
                    writer.writerow(
                        ["episode"]
                        + list(
                            range(1, args.total_episode + 1, args.eval_every_episode)
                        )
                    )
                    ...
                    if "active" in args.exp_mode:
                        ...
                        for i, norm_ent in enumerate(norm_ents):
                            ...
                            writer.writerow(
                                ["norm_ent {}".format(i)] + norm_ents_log[i]
                            )
                wandb.log(
                    {
                        "step": total_step_count_list[-1],
                        "eval_rew": eval_rews[-1],
                        "train_rew": train_rews[-1],
                    }
                )
            episode_rew = 0
            s_raw = env.reset()
