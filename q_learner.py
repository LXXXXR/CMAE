...
class Storage(object):
    ...
    def get_all_data(self):
        ...
        for i in inds:
            yield self.state[i], self.action[i], self.reward[i], self.next_state[
                i
            ], self.done[i]
    ...
    def get_goal_tractories(
        self, goal: (int, int), goal_replay_buffer: "Storage", base: int
    ):
        goal_s, goal_a = goal
        ...
        for ind in goal_s_inds:
            ind = ind[0]
            t = self.time_stamp[ind]
            goal_replay_buffer.batch_insert(
                self.state[ind - t : ind],
                self.action[ind - t : ind],
                self.reward[ind - t : ind],
                self.next_state[ind - t : ind],
                self.done[ind - t : ind],
            )
    ...
    def sample_states(self, bz):
        inds = np.random.choice(
            range(self.size), size=min(bz, self.size), replace=False
        )
...
class QLearning(object):
    def __init__(
        ...
        if all_subspace:
            self.counts = [
                [np.zeros((var_range, n_actions)) for var_range in nvec],  # 1 var
                [
                    np.zeros((nvec[0] * nvec[1], n_actions))
                    for _ in range(scipy.special.comb(len(nvec), 2, exact=True))
                ],  # 2 var
            ]
        if tree_subspace:
            self.counts = [
                [np.zeros((var_range, n_actions)) for var_range in nvec]
            ]  # 1 var
            for i in range(1, self.n_vars):
                self.counts.append(
                    [
                        None
                        for _ in range(
                            scipy.special.comb(self.n_vars, i + 1, exact=True)
                        )
                    ]
                )
            ...
            (
                self.counter_space_mapping,
                self.space_counter_mapping,
            ) = create_counter_space_mapping()
            ...
            self.counts = [
                ...
                [
                    np.zeros((nvec[0] * nvec[1], n_actions)),
                    np.zeros((np.prod(nvec[4:]), n_actions)),
                ],  # level 1
                [
                    np.zeros((np.prod(nvec[:4]), n_actions)),
                    np.zeros((nvec[0] * nvec[1] * np.prod(nvec[4:]), n_actions)),
                ],  # level 2
    ...
    def update_q(self, s, a, r, s_next, done):
        if not done:
            self.q_table[s, a] = (1 - self.alpha) * self.q_table[s, a] + self.alpha * (
                r + self.gamma * np.max(self.q_table[s_next])
            )
        else:
        ...
        if np.random.rand() < eps:
            ...
            if other_q_table is None:
                return np.random.choice(
                    np.flatnonzero(self.q_table[s] == self.q_table[s].max())
                )
            else:
    ...
    def update_count(
        self, s, a, multilevel=False, all_subspace=False, tree_subspace=False
    ):
        self.count[s, a] += 1
        if multilevel:
    ...
    def update_level_count(self, raw_s, a, level, all_subspace):
        level_reps = obs_to_level_int(
            raw_s, self.base, self.raw_dim, level=level, all_subspace=all_subspace
        )
...
class ExpBonusQLearning(QLearning):
    def __init__(
        ...
        super(ExpBonusQLearning, self).__init__(
            n_states, n_actions, base, raw_dim, observation_space, gamma, alpha
        )
...
class ActiveQLearning(QLearning):
    def __init__(
        ...
        all_subspace=False,
        no_range_info=False,
        stochastic_select_subspace=False,
        tree_subspace=False,
        recip_t=50,
        subspace_q_size=10,
        replay_size=50000,
        ...
        super(ActiveQLearning, self).__init__(
            n_states,
            n_actions,
            base,
            raw_dim,
            ...
            gamma,
            alpha,
            all_subspace=all_subspace,
            tree_subspace=tree_subspace,
            subspace_q_size=subspace_q_size,
        )
        self.replay_buffer = Storage(replay_size)
        self.goal_replay_buffer = Storage(
            replay_size
        )  # store trajectories that related to the goal
        self.base = base
            ...
            if self.n_updates % self.compute_ent_every == 0:
                if self.tree_subspace:
                    ...
                    if self.stochastic_select_subspace:
                        level_penalty = np.array(
                            [space[1] * self.level_penalty for space in self.subspaces]
                        )
                        exp_ent = np.exp(
                            -self.recip_t * np.array(norm_ents) + level_penalty
                        )
                        # print('exp_ent', exp_ent / np.sum(exp_ent))
                        selected_space_ids = np.random.multinomial(
                            1, exp_ent / np.sum(exp_ent)
                        )
                        selected_space = self.subspaces[
                            selected_space_ids.nonzero()[0].item()
                        ]
                    ...
                    if self.level < self.n_vars - 1:
                        ...
                        for var in range(self.n_vars):
                            ...
                            if self.counts[self.level + 1][count_id] is None:
                                subspace_var_dims = self.nvec[
                                    np.array(subspace_new, dtype=np.int32)
                                ]
                                self.counts[self.level + 1][count_id] = np.zeros(
                                    (np.prod(subspace_var_dims), self.n_actions)
                                )
                                self._init_counter_from_storage(
                                    self.level + 1, count_id
                                )
                    ...
                    if self.stochastic_select_subspace:
                        ...
                        selected_space = np.random.multinomial(
                            1, exp_ent / np.sum(exp_ent)
                        )
                        selected_space = selected_space.nonzero()[0].item()

                        if selected_space >= len(self.counts[0]):
                            self.level, self.count_id = 1, selected_space - len(
                                self.counts[0]
                            )
                        else:
                            self.level, self.count_id = 0, selected_space

            goal_s, goal_a = self.get_goal_proj(
                self.level, self.count_id, all_subspace=all_subspace
            )
        else:
    ...
    def _shape_replay_buffer(self, goal=None):
        ...
        if goal is None:
            ...
            count = np.where(
                state_count != 0, self.count, np.full_like(self.count, np.inf)
            )
    ...
    def update_q_from_D(self, epoch=1, reset_q=True, goal=None):
        ...
        if reset_q:
            ...
            success = self.replay_buffer.get_goal_tractories(
                goal, self.goal_replay_buffer, self.base
            )
        if success:
    ...
    def _prioritize_pos(self):
        ...
        for ind in inds:
            ...
            for _ in range(5):
                self.replay_buffer.batch_insert(
                    self.replay_buffer.state[ind - t : ind + 1],
                    self.replay_buffer.action[ind - t : ind + 1],
                    self.replay_buffer.reward[ind - t : ind + 1],
                    self.replay_buffer.next_state[ind - t : ind + 1],
